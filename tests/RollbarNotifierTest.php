...
class RollbarNotifierTest extends PHPUnit_Framework_TestCase {
    ...
    public function testCheckIgnore() {
        ...
        $config['checkIgnore'] = function ($isUncaught, $callerArgs, $payload) {
        ...
        $this->assertNull($notifier->reportException(new \Exception('test exception')));

        // Shouldn't ignore this exception.
        $_SERVER = array();
        $this->assertValidUUID($notifier->reportException(new \Exception('test exception')));
    }
